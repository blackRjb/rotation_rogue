...
if menu.combo_box then
    menu.build = menu.combo_box.new()
else
    -- Si menu.combo_box n'existe pas, utilisez une alternative ou affichez un message d'erreur
    print("Error: menu.combo_box is not defined")
    menu.build = {} -- ou une autre valeur par défaut appropriée
end

on_render_menu(function()
    if not menu.main_tree:push("Rogue: Base") then
        return
    end

    local enabled = menu.main_boolean:render("Enable Plugin", "")
    
    -- Afficher l'état actuel de la case à cocher pour le débogage
    print("Enable Plugin state:", enabled)

    if enabled then
        -- Ajout de l'option de build
        local build_options = {"Andariels", "Rapid Fire", "Others"}
        menu.build:render("Build", build_options, "Select your build")
        ...
        menu.main_tree:separator()

        -- Ajoutez des vérifications pour s'assurer que create_spell_submenu est définie
        if type(create_spell_submenu) == "function" then
            create_spell_submenu("Basic Abilities", {"concealment", "caltrop", "puncture", "heartseeker", "forceful_arrow"})
            create_spell_submenu("Advanced Abilities", {"blade_shift", "invigorating_strike", "twisting_blade", "barrage", "rapid_fire", "flurry", "penetrating_shot"})
            create_spell_submenu("Movement Abilities", {"dash", "shadow_step"})
            create_spell_submenu("Utility Abilities", {"smoke_grenade", "poison_trap", "dark_shroud"})
            create_spell_submenu("Imbuements", {"shadow_imbuement", "poison_imbuement", "cold_imbuement"})
            create_spell_submenu("Ultimate Abilities", {"shadow_clone", "death_trap", "rain_of_arrows"})
        else
            print("Error: create_spell_submenu is not defined")
        end
    else
